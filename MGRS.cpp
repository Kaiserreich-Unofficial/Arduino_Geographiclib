...
namespace GeographicLib
{
                     ...
                     int prec, std::string &mgrs)
  {
    ...
    if (!(zone >= UTMUPS::MINZONE && zone <= UTMUPS::MAXZONE))
      assert("Zone " + Utility::str(zone) + " not in [0,60]");
    if (!(prec >= -1 && prec <= maxprec_))
      assert("MGRS precision " + Utility::str(prec) + " not in [-1, " + Utility::str(int(maxprec_)) + "]");
    ...
    if (utmp)
    {
      ...
      if (irow != yh - (northp ? minutmNrow_ : maxutmSrow_))
        assert("Latitude " + Utility::str(lat) + " is inconsistent with UTM coordinates");
                     ...
                     int &prec, bool centerp)
  {
    ...
    if (p > 0 && !(zone1 >= UTMUPS::MINUTMZONE && zone1 <= UTMUPS::MAXUTMZONE))
      assert("Zone " + Utility::str(zone1) + " not in [1,60]");
    if (p > 2)
      assert("More than 2 digits at start of MGRS " + mgrs.substr(0, p));
    if (len - p < 1)
      assert("MGRS string too short " + mgrs);
    bool utmp = zone1 != UTMUPS::UPS;
    int zonem1 = zone1 - 1;
    const char *band = utmp ? latband_ : upsband_;
    int iband = Utility::lookup(band, mgrs[p++]);
    if (iband < 0)
      assert("Band letter " + Utility::str(mgrs[p - 1]) + " not in " + (utmp ? "UTM" : "UPS") + " set " + band);
    ...
    { // Grid zone only (ignore centerp)
      ...
      assert("Missing row letter in " + mgrs);
    ...
    if (icol < 0)
      assert("Column letter " + Utility::str(mgrs[p - 1]) + " not in " + (utmp ? "zone " + mgrs.substr(0, p - 2) : "UPS band " + Utility::str(mgrs[p - 2])) + " set " + col);
    int irow = Utility::lookup(row, mgrs[p++]);
    if (irow < 0)
      assert("Row letter " + Utility::str(mgrs[p - 1]) + " not in " + (utmp ? "UTM" : "UPS " + Utility::str(hemispheres_[northp1])) + " set " + row);
    if (utmp)
    {
      if (zonem1 & 1)
      ...
      if (irow == maxutmSrow_)
        assert("Block " + mgrs.substr(p - 2, 2) + " not in zone/band " + mgrs.substr(0, p - 2));
    ...
    for (int i = 0; i < prec1; ++i)
    {
      ...
      if (ix < 0 || iy < 0)
        assert("Encountered a non-digit in " + mgrs.substr(p));
      x1 = base_ * x1 + ix;
      y1 = base_ * y1 + iy;
    }
    if ((len - p) % 2)
    {
      if (Utility::lookup(digits_, mgrs[len - 1]) < 0)
        ...
        assert("Not an even number of digits in " + mgrs.substr(p));
    }
    if (prec1 > maxprec_)
      assert("More than " + Utility::str(2 * maxprec_) + " digits in " + mgrs.substr(p));
    if (centerp)
    {
      unit *= 2;
      x1 = 2 * x1 + 1;
  ...
  void MGRS::CheckCoords(bool utmp, bool &northp, real &x, real &y)
  {
    ...
    if (!(ix >= mineasting_[ind] && ix < maxeasting_[ind]))
    {
      if (ix == maxeasting_[ind] && x == maxeasting_[ind] * tile_)
        ...
        assert("Easting " + Utility::str(int(floor(x / 1000))) + "km not in MGRS/" + (utmp ? "UTM" : "UPS") + " range for " + (northp ? "N" : "S") + " hemisphere [" + Utility::str(mineasting_[ind] * tile_ / 1000) + "km, " + Utility::str(maxeasting_[ind] * tile_ / 1000) + "km)");
    }
    if (!(iy >= minnorthing_[ind] && iy < maxnorthing_[ind]))
    {
      if (iy == maxnorthing_[ind] && y == maxnorthing_[ind] * tile_)
        ...
        assert("Northing " + Utility::str(int(floor(y / 1000))) + "km not in MGRS/" + (utmp ? "UTM" : "UPS") + " range for " + (northp ? "N" : "S") + " hemisphere [" + Utility::str(minnorthing_[ind] * tile_ / 1000) + "km, " + Utility::str(maxnorthing_[ind] * tile_ / 1000) + "km)");
    }
  ...
  int MGRS::UTMRow(int iband, int icol, int irow)
  {
    ...
    //     9     80     94
    int
        minrow = iband > -10 ? int(floor(c - real(4.3) - real(0.1) * northp)) : -90,
                    ...
                    string &easting, string &northing)
  {
    ...
    if (p0 == string::npos)
      assert("MGRS::Decode: ref does not contain alpha chars");
    if (!(p0 <= 2))
      assert("MGRS::Decode: ref does not start with 0-2 digits");
    string::size_type p1 = mgrs.find_first_of(alpha_, p0);
    if (p1 != p0)
      assert("MGRS::Decode: ref contains non alphanumeric chars");
    p1 = min(mgrs.find_first_not_of(alpha_, p0), n);
    if (!(p1 == p0 + 1 || p1 == p0 + 3))
      assert("MGRS::Decode: ref must contain 1 or 3 alpha chars");
    if (p1 == p0 + 1 && p1 < n)
      assert("MGRS::Decode: ref contains junk after 1 alpha char");
    if (p1 < n && (mgrs.find_first_of(digits_, p1) != p1 ||
                   mgrs.find_first_not_of(digits_, p1) != string::npos))
      assert("MGRS::Decode: ref contains junk at end");
    if ((n - p1) & 1u)
      assert("MGRS::Decode: ref must end with even no of digits");
  ...
  void MGRS::Check()
  {
    ...
    if (!(lon < 0))
      assert("MGRS::Check: equator coverage failure");
    UTMUPS::Reverse(31, true, 1 * t, 95 * t, lat, lon);
    if (!(lat > 84))
      assert("MGRS::Check: UTM doesn't reach latitude = 84");
    UTMUPS::Reverse(31, false, 1 * t, 10 * t, lat, lon);
    if (!(lat < -80))
      assert("MGRS::Check: UTM doesn't reach latitude = -80");
    UTMUPS::Forward(56, 3, zone, northp, x, y, 32);
    if (!(x > 1 * t))
      assert("MGRS::Check: Norway exception creates a gap");
    UTMUPS::Forward(72, 21, zone, northp, x, y, 35);
    if (!(x > 1 * t))
      assert("MGRS::Check: Svalbard exception creates a gap");
    UTMUPS::Reverse(0, true, 20 * t, 13 * t, lat, lon);
    if (!(lat < 84))
      assert("MGRS::Check: North UPS doesn't reach latitude = 84");
    UTMUPS::Reverse(0, false, 20 * t, 8 * t, lat, lon);
    if (!(lat > -80))
      assert("MGRS::Check: South UPS doesn't reach latitude = -80");
    ...
    for (int i = 0; i < bandchecks; ++i)
    {
      ...
      if (!(LatitudeBand(lat) == tab[3 * i + 0]))
        assert("MGRS::Check: Band error, b = " +
