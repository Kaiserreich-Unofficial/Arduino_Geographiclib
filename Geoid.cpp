...
namespace GeographicLib
{
  ...
  Geoid::Geoid(const std::string &name, const std::string &path, bool cubic,
               bool threadsafe)
      : _name(name), _dir(path), _cubic(cubic), _a(Constants::WGS84_a()), _e2((2 - Constants::WGS84_f()) * Constants::WGS84_f()), _degree(Math::degree()), _eps(sqrt(numeric_limits<real>::epsilon())), _threadsafe(false) // Set after cache is read
  {
    ...
    if (!(_file.good()))
      assert("File not readable " + _filename);
    string s;
    if (!(getline(_file, s) && s == "P5"))
      assert("File not in PGM format " + _filename);
    ...
    while (getline(_file, s))
    {
      ...
      if (s[0] == '#')
      {
        ...
        {
          if (!(is >> _offset))
            assert("Error reading offset " + _filename);
        }
        else if (key == "Scale")
        {
          if (!(is >> _scale))
            assert("Error reading scale " + _filename);
        }
      ...
      else
      {
        ...
        if (!(is >> _width >> _height))
          assert("Error reading raster size " + _filename);
        break;
      }
    }
    {
      unsigned maxval;
      if (!(_file >> maxval))
        assert("Error reading maxval " + _filename);
      if (maxval != pixel_max_)
        assert("Incorrect value of maxval " + _filename);
    ...
    if (_offset == numeric_limits<real>::max())
      assert("Offset not set " + _filename);
    if (_scale == 0)
      assert("Scale not set " + _filename);
    if (_scale < 0)
      assert("Scale must be positive " + _filename);
    if (_height < 2 || _width < 2)
      // Coarsest grid spacing is 180deg.
      assert("Raster size too small " + _filename);
    if (_width & 1)
      // This is so that longitude grids can be extended thru the poles.
      assert("Raster width is odd " + _filename);
    if (!(_height & 1))
      // This is so that latitude grid includes the equator.
      assert("Raster height is even " + _filename);
    ...
    if (!_file.good() ||
      ...
      assert("File has the wrong length " + _filename);
  ...
  {
    if (_threadsafe)
      assert("Attempt to change cache of threadsafe Geoid");
    if (south > north)
    {
      CacheClear();
      return;
    ...
    for (int iy = in; iy <= is; ++iy)
    {
      ...
      filepos(iw1, iy1);
      Utility::readarray<pixel_t, pixel_t, true>(_file, &(_data[iy - in][0]), xs1);
      if (xs1 < _xsize)
      {
        // Wrap around longitude = 0
        filepos(0, iy1);
        Utility::readarray<pixel_t, pixel_t, true>(_file, &(_data[iy - in][xs1]), _xsize - xs1);
      }
    }
    _cache = true;
  }
